...
export async function resolveProvider(installationId: number) {
  ...
  const config = { provider, model: ic?.model || cfg.defaultModel, endpoint: ic?.endpoint, apiKeyPlain: (provider==='openai') ? cfg.openaiKey : undefined, ghModelsToken: undefined };
